#This script is used to calculate the association between two nominal variables or
#between one nominal and one ordinal variable
...
NomOrdinalTest <- function(Features)
{
  ...
  for (loop in 2: ncol(Features))
  {
    ...
    out[loop-1,] = extract.fun(coin::chisq_test(tbw, scores = list("pred"=1:nlevels(factor(Features[,loop])))))
...
NomNomTest <- function(Features)
{
  ...
  {
    tbw  = table(relapse=Features[,1],pred=Features[,loop])
    out[loop-1,] = extract.fun(coin::chisq_test(tbw))
